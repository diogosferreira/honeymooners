export function swiperTestimonials() {
    const element = document.querySelector(".testimonial-share_component");
    if (!element) return;


    // Count the number of elements with the class '.testimonial-share_component'
    const testimonialCount = document.querySelectorAll(
    ...
    function updateProgressBar(currentSlide, swiper) {
        ...
        if (progressBar) {
            ...
            console.warn(
                "Progress bar element '.carroussel-current-line' not found."
            );
        }
    }
